...
Kernel_pca_f = KernelPCA(n_components=kernel_pca_comp*10, kernel="rbf")
...
xgb_classifier = xgb.XGBClassifier(n_estimators=10*kernel_pca_comp,
                                   reg_alpha=.5,
                                   max_depth=1,
...
''' build gpt'''
train_dataset = prepare_phoneme_dataset(data_in.astype('int'), data_out.astype('int'), vocab_size=vocab_size)
...
train_config.learning_rate = 1e-3 # the gpt_model we're using is so small that we can go a bit faster
...
for ii in np.random.randint(0,data_in.shape[0],prediction_result.shape[0]):
    input_length =  np.random.randint(15,20)
    ...
    prediction_result[p, 0] = data_out[ii][input_length-1 ]
    prediction_result[p, 1] = (y.detach().cpu()[0, -1])
    # print('-' * 80)
    p =p+1
print('accuracy= %f', accuracy_score(prediction_result[:,0],prediction_result[:,1])*100)

######################################### Finalize Result and correction with gpt #######
PL0 = first_y_tri.mean(axis=0)
PL0=PL0/PL0.sum()
...
def correct_D4_result_gpt(model, st,y_true,y_predic_probs, PL0 ):
    ...
    for cc, ii_snt in enumerate(np.unique(st).astype('int')):
        ...
        P_pp = y_predic_probs[sent_indexes]
        P_lang = np.zeros_like(P_pp)
        P_total = np.zeros_like(P_pp)

        P_total[0, :] = np.multiply(PL0, P_pp[0, :])
        P_lang[0, :] = PL0
        p_z=np.ones_like(PL0)/vocab_size
        for ii_wrd in range(1,len(y_sent)):

            if ii_wrd ==1:
                x = torch.tensor(np.argmax(P_total[:ii_wrd, :]).reshape([1,-1]), dtype=torch.long).to(trainer.device)
                # x = torch.tensor(y_sent[:ii_wrd].reshape([1,-1]), dtype=torch.long).to(
                # trainer.device)
            else:

                x = torch.tensor(np.argmax(P_total[:ii_wrd, :], axis=1).reshape([1,-1]), dtype=torch.long).to(trainer.device)
                # x = torch.tensor(y_sent[:ii_wrd].reshape([1,-1]), dtype=torch.long).to(
                #     trainer.device)
            x = x.expand(num_samples_langmodel, -1)
            y, probs = model.generate(x, max_new_tokens=1, do_sample=do_sample_L, top_k=40)
            P_lang[ii_wrd, :] = probs.squeeze().mean(axis=0).numpy()
            P_total[ii_wrd, :] = (ii_wrd/y_sent.shape[0])*P_lang[ii_wrd, :] +((y_sent.shape[0]-ii_wrd)/y_sent.shape[0])*P_pp[ii_wrd, :]
            # P_total[ii_wrd, :]=P_total[ii_wrd, :]/P_total[ii_wrd, :].sum()
            # p_z=P_pp[ii_wrd, :]+1e-6
            # p_z=p_z/p_z.sum()
        if cc == 0:
            P_all = P_total
            y_all = y_sent
