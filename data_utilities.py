...
def get_data(patient_id, datasets_add, feature_id):
    '''
    ...
    new_phones_df = pd.DataFrame(
        temp_frame.repeat(phones_df.onset[0]).reshape([-1, phones_df.onset[0]]).transpose(),
        columns=phones_df.columns)
    new_phones_df = new_phones_df.append(pd.DataFrame(
        phones_df.iloc[0].values.repeat(phones_df.duration[0]).reshape([-1, phones_df.duration[0]]).transpose(),
        columns=phones_df.columns))
    for ii in range(1, phones_df.shape[0]):
        ...
        if phones_df.duration[ii - 1] + phones_df.onset[ii - 1] < phones_df.onset[ii]:
            ...
            new_phones_df = new_phones_df.append(pd.DataFrame(
                phones_df.iloc[ii].values.repeat(numb_rep).reshape(
                    [-1, numb_rep]).transpose(), columns=phones_df.columns))
        else:
            pass

        new_phones_df = new_phones_df.append(pd.DataFrame(
            phones_df.iloc[ii].values.repeat(phones_df.duration[ii]).reshape(
                [-1, phones_df.duration[ii]]).transpose(), columns=phones_df.columns))
    ...
    return total_data, neural_df, phones_df, new_phones_df, trials_df, dt, zero_time
